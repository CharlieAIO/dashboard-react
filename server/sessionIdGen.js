...
var finalString = "";
var cnt = 0;
window.io_install_flash = !1,  //PROVERITI DA LI OVO TREBA LOADOVATI ISTO!!!!!!
    window.io_install_stm = !1,
    window.io_exclude_stm = 12,
    window.io_enable_rip = !0;
...
var _i_d = {
    ...
    __if_t: function (_i_al) {
        ...
        for (var _i_g = 0; _i_g < _i_al.length; _i_g++) {
            ...
            if (_i_h >= 56320 && _i_h < 57344)
                continue;
            if (_i_h >= 55296 && _i_h < 56320) {
                if (_i_g + 1 >= _i_al.length)
                    continue;
                var _i_i = _i_al.charCodeAt(++_i_g);
                if (_i_i < 56320 || _i_h >= 56832)
                    continue;
                _i_h = ((_i_h - 55296) << 10) + (_i_h - 56320) + 65536;
            }
            if (_i_h < 128)
                _i_f += String.fromCharCode(_i_h);
            else if (_i_h < 2048)
                _i_f += String.fromCharCode(192 + (_i_h >> 6), 128 + (_i_h & 63));
            else if (_i_h < 65536)
                _i_f += String.fromCharCode(224 + (_i_h >> 12), 128 + ((_i_h >> 6) & 63), 128 + (_i_h & 63));
            else
                _i_f += String.fromCharCode(240 + (_i_h >> 18), 128 + ((_i_h >> 12) & 63), 128 + ((_i_h >> 6) & 63), 128 + (_i_h & 63));
        }
        return _i_f;
    },
...
var _i_o = {
    ...
    __if_ap: function (_i_al) {
        ...
        do {
            ...
            _i_x = chr2 = chr3 = "";
            _i_s = _i_t = _i_u = _i_v = "";
        } while (_i_g < _i_al.length); return _i_w;
    }
};
var _i_z = {
    _i_fu: false,
...
if (window.io_stm_cab_url === undefined)
    window.io_stm_cab_url = _i_o.__if_ap("aHR0cHM6Ly9tcHNuYXJlLmllc25hcmUuY29tLw==") + "/StmOCX.cab";
...
if (typeof (window.io_enable_rip) != "boolean")
    window.io_enable_rip = _i_z._i_fz;
...
var _i_aa = {
    getElementById: function (_if_gw) {
        if (_if_gw === undefined)
            return null;
        if (typeof (_if_gw) == "object" && _if_gw.tagName)
            return _if_gw;
        ...
        if (document.getElementById)
            return document.getElementById(_if_gw);
        return null;
    }
};
var _i_ac = {
...
var _i_cr = {
    ...
    __if_ej: function (_if_gr) {
        if (typeof (_if_gr) != "string" || (this._i_gk > 0 && _if_gr.length > this._i_gk))
            return;
    ...
    __if_fw: function (_if_he) {
        ...
        } while (_i_cx < _if_he.length); this.__if_gr(true);
    },
    __if_gr: function (_if_hf) {
        try {
            if (_if_hf || __if_i()) {
                ...
                var load = this.__if_cz();
                if (firstItem != undefined && firstItem.length > 1 && cnt == 1) {
                    finalString = load;
                    cnt++;
                }
                else
...
function __if_d() {
    this.version = appVersion.match(/[^\s]*\s?/g)[0];
...
__if_d.prototype = {
    __if_hn: function () {
        ...
        this.products = userAgent.match(/([\w]+\s)?[^\s\/]*\/[^\s]*/g);
    },
    __if_il: function () {
        ...
        for (_i_g = 0; _i_g < _i_dd.length; _i_g++) {
            for (_i_bl = 0; this.attributes && _i_bl < this.attributes.length; _i_bl++) {
                if (this.attributes[_i_bl].toUpperCase().search(_i_dd[_i_g].toUpperCase()) >= 0) {
                    ...
                    if (this.version == this.attributes[_i_bl])
                        this.version = "";
                    if (_i_g > 0) {
...
var io_adp = {
    ...
    readyStateChangeHandler: function () {
        ...
        if (this._i_gl || _i_do.readyState != 4)
            return;
        try {
...
__if_e.prototype = {
    __if_op: function () {
        ...
        this._i_gm = true;
        try {
            if (false && window.openDatabase && !(window[_i_a] && window[_i_a]['WDBLock'])) {
...
var _i_dt = {
    ...
    __if_sm: function (_i_go, _i_e, _i_dr) {
        if (typeof (_i_dr) != 'undefined' && _i_dr)
            _i_go.executeSql('INSERT INTO tokens (token) VALUES (?)', [_i_dr], null, _i_dt.__if_rd);
    },
    __if_tw: function (_i_go, _i_dr) {
        _i_go.executeSql('SELECT * FROM tokens', [], function (_i_go, _if_hi) {
            if (_if_hi.rows && _if_hi.rows.length > 0) {
                ...
                _i_dt.__if_sm(_i_go, _if_hi, _i_dr);
        }, _i_dt.__if_rd);
    },
...
function iov_fl_cb(_if_hk, _if_hj, _i_gn) {
    for (var _i_dw in io_cm) {
        if (typeof (_i_gn) != "undefined" && io_cm[_i_dw]._i_gn == _i_gn) {
            if (!io_cm[_i_dw]._i_gl)
                io_cm[_i_dw]._i_gl = _if_hj;
    ...
    if (_if_hk != "0000")
        _i_cr.__if_fw(_if_hk);
    else
        _i_cr.__if_gr(_if_hj);
    return true;
}
...
__if_g.prototype = {
    ...
    __if_op: function () {
        ...
        if (!_i_ev || this.__if_abn(_i_ev) || (userAgent.toLowerCase().indexOf("firefox") > -1 && !(this.__if_ade(_i_ev))) || parseFloat(_i_ev) < _i_eu) {
            try {
...
var io_dp = {
    ...
    __if_op: function () {
        ...
        try {
            ...
            _i_cr.__if_fc("JSSRC", _i_o.__if_ap(inJSSRC));
            _i_cr.__if_fc("UAGT", userAgent.slice(0, 400));
            if (true) {
                _i_cr.__if_fc("JSTOKEN", jsTOKEN);
            }

            var _i_fd = decodeURIComponent(agentEncoded);
            if (false && userAgent != _i_fd) {
                _i_cr.__if_fc("JDIFF", "1");
                _i_cr.__if_fc("SUAGT", _i_fd.slice(0, 400));
            }
            _i_cr.__if_fc("HACCLNG", decodeURIComponent(languageEncoded));
            _i_cr.__if_fc("HACCCHR", decodeURIComponent(""));
            _i_cr.__if_fc("JSVER", "3.1.3");
            _i_cr.__if_fc("TZON", String(aexString)); 
            
           // var dateFormat = require('dateformat');

            var _i_fe = new Date();

            var UTCtime = _i_fe.setMinutes(_i_fe.getMinutes() + _i_fe.getTimezoneOffset());
            var getNewTime = new Date(UTCtime);
            getNewTime.setMinutes(getNewTime.getMinutes() - timezoneSet);

            //var getDate = dateFormat(UTCtime, "yyyy/mm/dd HH:MM:ss");

            var shiftDate = new Date();
            shiftDate.setSeconds(shiftDate.getSeconds() + between(8, 15));

           _i_cr.__if_fc("JSTIME", _i_d.__if_o(shiftDate)); // na osnovu timezone-a koji se prosledi kao parametar, odrediti trenutno vreme
            // _i_cr.__if_fc("JSTIME", "2020/11/03 23:31:27");
             _i_cr.__if_fc("SVRTIME", inSVRTIME);
            ...
            _i_cr.__if_fc("APVER", appVersion);
            _i_cr.__if_fc("APNAM", appName);
            _i_cr.__if_fc("OSCPU", undefined);
            _i_cr.__if_fc("NPLAT", appPlatform);
            ...
            _i_cr.__if_fc("JLANG", "en-US");
            _i_cr.__if_fc("JCOX", "");
            _i_cr.__if_fc("IGGY", inIGGY);
            _i_cr.__if_fc("JRES", dimensionHeight + "x" + dimensionWidth);
            _i_cr.__if_fc("JSMBR", "");
            _i_cr.__if_fc("XREQW", decodeURIComponent(""));

            _i_cr.__if_fc("JPLGNS", allPlugins);
            _i_cr.__if_fc("JREFRR", doReferrer);
        } catch (e) {
            console.log(e);
    ...
    updateBlackboxes: function () {
        if (!__if_h()) {
            if (io_dp.CTOKEN)
                _i_cr.__if_fc("CTOKEN", io_dp.CTOKEN);
            _i_cr.__if_gr(true);
        }
    }
};
...
exports.initMAIN = function initMain(getinJSSRC, getuserAgent, getjsTOKEN, getagentEncoded, getlanguageEncoded, getaexString, getinSVRTIME, getBrowser, getappVersion, getosName, getappName, getappPlatform, getinIGGY, getdimensionHeight, getdimensionWidth, getallPlugins, getdoReferrer, getintLoc, gettimezoneSet) {
    inJSSRC = getinJSSRC;
    userAgent = getuserAgent;
    jsTOKEN = getjsTOKEN;
    agentEncoded = getagentEncoded;
    languageEncoded = getlanguageEncoded;
    aexString = getaexString;
    inSVRTIME = getinSVRTIME;
    ...
    inIGGY = getinIGGY;
    dimensionHeight = getdimensionHeight;
    dimensionWidth = getdimensionWidth;
    allPlugins = getallPlugins;
    doReferrer = getdoReferrer;
    intLoc = getintLoc;
    timezoneSet = gettimezoneSet;
    startSessionIDgen();
    return finalString;
}
function startSessionIDgen() {
    try {
        _i_cr.__if_fc("INTLOC", intLoc);

        var _i_fm = new __if_f("io_temp");
        var _i_fn = new __if_e("io_temp");
        var _i_fo = new __if_g(_i_o.__if_ap("aHR0cHM6Ly9tcHNuYXJlLmllc25hcmUuY29tLw==") + "stmgwb2.swf", false ? "" : jsTOKEN); io_cm.push(_i_fm, _i_fn, io_adp, _i_fo);
        if (false)
            _i_cr.__if_fc("FLRTD", "tGUF7feOvBY3yecMkbF6wzUKsbc4EEdBkpZnqT+xIs0=");
        else
            _i_fm._if_hn = _i_fn._if_hn = jsTOKEN;
        if (false) {
            try {
                var _i_dl = document.getElementsByTagName('head')[0];
...
if (false)
    document.addEventListener("DOMContentLoaded", __if_l, false);
(function __if_n() {
